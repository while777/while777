function client2(S1, S2, S3, r, T, sigma1, sigma2, sigma3, rho12, rho13, rho23, N, seed)

rng(seed);
nu1 = r-sigma1.^2./2 ; nu2=r-sigma2.^2./2; nu3=r-sigma3.^2./2;
...
c = 1000000 * max( 3/7 * min((S1T/S1), 1.38) + 1/7 .* min((S2T/S2), 1.38) + 3/7 .* min((S3T/S3), 1.38)    , 1.32);
c2 = mean(c)
c0 = exp(-r * T) * 100 * max( 3/7 * min((S1T/S1), 1.38) + 3/7 .* min((S2T/S2), 1.38) + 3/7 .* min((S3T/S3), 1.38)    , 1.32);
c1 =mean(c0)
%
