function [Mean1, Std] = controlvariateMC(S1,S2,S3,q1,q2,q3, r, T,sigma1,sigma2,sigma3,rho12,rho23,rho13,N,seed)
%%
tic
rng(seed);
...
cv = exp(-r.*T).* 100 * max(1/3 * (S1T/S1 + S2T/S2 + S3T/S3) ,1);
Mean = mean(cv);
Varcov = cov(c, cv); 
coe =  Varcov(1, 2)./ var(cv);
CV = c - coe .* (cv - Mean);
Mean1 = mean(CV);
Std = std(CV)
toc
