function [price, Std, CI] = antitheticMC(S1,S2,S3,q1,q2,q3, r,T,sigma1,sigma2,sigma3,rho12,rho23,rho13,N,seed)
tic
rng(seed);
...
%
s1T=S1* exp(nu1T+sigma1sqrtT.*X1(1,:));
s2T=S2* exp(nu2T+sigma2sqrtT.*X1(2,:));
s3T=S3* exp(nu3T+sigma3sqrtT.*X1(3,:));
%
c = exp(-r.*T).* 100 * max(1/3 * min(S1T/S1, 1.25) + 1/3 * min(S2T/S2, 1.25) + 1/3 * min(S3T/S3, 1.25)   ,1);
c1 = exp(-r.*T).* 100 * max(1/3 * min(s1T/S1, 1.25) + 1/3 * min(s2T/S2, 1.25) + 1/3 * min(s3T/S3, 1.25)  ,1);
c2 = 0.5 * (c + c1);

[MUHAT,SIGMAHAT,MUCI,SIGMACI]=normfit(c2);
CI= (MUCI(2)-MUCI(1))/2
